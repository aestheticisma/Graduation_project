...
def savefile(samples):
    for i,sample in enumerate(samples):
        ...
        with open("bugreports_sds/" + str(i+1)+".txt", "w", encoding='utf-8') as f:
            f.write(string)

# filename = "goldset.txt"
# label_list = read_label(filename)
def get_ids():
    filename = "./data/bugreports_sds.xml"
    bugreports = read_xml(filename)
    samples, ids = get_content(bugreports)
    # savefile(samples)
    return ids